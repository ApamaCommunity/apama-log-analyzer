...
class PySysTest(AnalyzerBaseTest):
	...
	def validate(self):
		...
		last = data['status'][-1]
		last_4_3_2_0=last
		self.assertEval("{last_4_3_2_0}['datetime'].startswith('2019-09-')", last_4_3_2_0=last)
		self.assertEval("len({zero_values_in_4_3_2_0})==0", zero_values_in_4_3_2_0=[k for k in last if k!='datetime' and last[k]<=0])
		...
		with io.open(outputdir+'/status_correlator-10.3.1.0.json') as f:
			data = json.load(f) # check it's a valid json document
		last = data['status'][-1]
		self.assertEval("{last_new}['datetime'].startswith('2019-04-')", last_new=last)
