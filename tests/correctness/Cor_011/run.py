...
class PySysTest(AnalyzerBaseTest):
	...
	def validate(self):
		...
		self.assertDiff('loganalyzer_java_output/logged_errors.txt', 'ref-java-errors.txt', ignores=['log analyzer v'])
		self.assertDiff('loganalyzer_java_output/logged_warnings.txt', 'ref-java-warnings.txt', ignores=['log analyzer v'])
		self.assertDiff('loganalyzer_nonjava_output/logged_errors.txt', 'ref-nonjava-errors.txt', ignores=['log analyzer v'])