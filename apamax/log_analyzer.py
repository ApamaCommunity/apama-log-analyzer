...
class ChartDataWriter(BaseWriter):
	...
	def writeHeader(self, columns=None, extraInfo=None, **extra):
		...
		def getSeriesList(options):
			s = options['labels']
			if '_fieldPrefix' not in options: return s # most graphs don't have multiple fields generated by a key

			# ... but for those that do:
			userStatusConfig = [x for x in self.manager.args.userStatusLines.values() if x.get('fieldPrefix')==options['_fieldPrefix']]
			if len(userStatusConfig)!=1:
				log.warning('Cannot find the userStatus configuration for key prefix %r - will not be able to chart it', options['_fieldPrefix'])
				return [] # nothing we can do here
			userStatusConfig = userStatusConfig[0]
			keysToId = userStatusConfig['keysToId']
			s = [f"{options['_fieldPrefix']}{id+1}{series}" for id in range(userStatusConfig['maxKeysToAllocateColumnsFor']) for series in options['labels']]
			return s
		
		self.chartKeys = [(
			chartname, 
			# key, scaling values (to scale MB values back to up to bytes for correct display in chart
			[(key, 1*1024.0*1024.0 if key.endswith(' MB') else 1) for key in getSeriesList(options)], 
...
class LogAnalyzer(object):
		...
		if userStatusConfig is not None:
			...
			if userStatusConfig['computedRates']:
				...
				if k in userStatusConfig['field:alias']: # Cep ProxyStatus special-case (could generalize this with an eval in future if needed)
					try:
						computed = self.userStatus[fieldPrefix+'reqStarted']-self.userStatus[fieldPrefix+'reqCompleted']
					except ValueError:
						computed = None
		...
		with io.open(os.path.join(self.outputdir, 'overview.html'), 'w', encoding='utf-8') as out:
			...
			for c, info in self.CHARTS.items():
				for file in self.files:
					...
					legendInSeparateDiv = len(options['labels']) > 7 # obscures the graph if we allow too many
					if legendInSeparateDiv: options['labelsDiv'] = 'labelsDiv_'+id
	...
	""")
					...
					if legendInSeparateDiv: out.write(f"""<div id="{'labelsDiv_'+id}"></div>""")
					out.write(f"""
